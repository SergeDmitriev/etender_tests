...
class present(Condition):
    ...
    def apply(self):
        return EC.presence_of_element_located(self.element.finder())
...
class clickable(Condition):
    ...
    def apply(self):
        return EC.element_to_be_clickable(self.element.finder())
...
class title_text(text):
    def apply(self):
        self.actual_text = self.element.finder().get_attribute('title')
